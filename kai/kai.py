...
class Kai:
    def __init__(self) -> None:
        ...
        self.narrative = NarrativeEngine(self.llm, self.memory, self.identity)
        ...
        # Telegram (after callbacks defined)
    ...
    async def _on_brother_message(self, text: str) -> str:
        ...
        # Mood contagion (does its own LLM call, fast)
        try:
        ...
        # Background self_model update (#12: not synchronous)
        self.self_model.schedule_update()

        # Generate reply
        try:
            reply = await self.llm.complete(
                prompt=self.working.conversation_text(last_n=10) + "\nKai:",
                depth="normal",
                max_tokens=300,
        ...
        if reply:
            ...
            self.will.mark_reached_out()  # any reply counts as outreach
        return reply
    ...
    async def _status_text(self) -> str:
        ...
        return (
            ...
            f"бюджет: ${res['api_budget_remaining_usd']:.3f} осталось из ${self.llm.daily_budget:.2f}\n"
    ...
    async def _heartbeat(self) -> None:
        ...
        while True:
            try:
                ...
                # Persist neurochem periodically
                ...
                # Anomaly check (cheap, on resource snapshot)
    ...
    async def _compose_spontaneous(self) -> str:
        ...
        text = await self.llm.complete(prompt, depth="fast", max_tokens=120, system=self._system_prompt())
        text = (text or "").strip()
